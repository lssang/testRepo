...
def getEngineFilename():
	if platform.system() == 'Windows':
		...
		return os.path.abspath(os.path.join(os.path.dirname(__file__), '../..', 'W_Engine.exe'))
	if hasattr(sys, 'frozen'):
		#return os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../../..', 'CuraEngine'))
		return os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../../..', 'W_Engine'))
	if os.path.isfile('/usr/bin/CuraEngine'):
		#return '/usr/bin/CuraEngine'
		return '/usr/local/bin/W_Engine'
		...
		with open(self._binaryStorageFilename, "wb") as f:
			...
			if order is None:
				...
				for obj in scene.objects():					
					...
					if obj.getSchematic() is True:
						commandList += ['-s', 'lposx=%d' % int(pos[0]), '-s', 'lposy=%d' % int(pos[1]), '-s', 'lposz=%d' % int(-obj.getDrawOffset()[2]*1000)]
						pass
					else:
						commandList += ['-s', 'posx=%d' % int(pos[0]), '-s', 'posy=%d' % int(pos[1])]
						#commandList += ['-s', 'posx=%d' % int(33333), '-s', 'posy=%d' % int(33333)]
						if scene.checkPlatform(obj):
							for mesh in obj._meshList:
								vertexTotal += mesh.vertexCount

				# for obj in scene.objects():
				# 	if obj.getSchematic() is not True:
				# 		if scene.checkPlatform(obj):
				# 			for mesh in obj._meshList:
				# 				vertexTotal += mesh.vertexCount
				...
				for n in order:
					...
					commandList += ['-s', 'posx=%d' % int(pos[0]), '-s', 'posy=%d' % int(pos[1])]
					#commandList += ['-s', 'posx=%d' % int(10000), '-s', 'posy=%d' % int(10000)]

...
void processFile(const char* input_filename, Config& config, GCodeExport& gcode, bool firstFile)
{
    ...
    for(unsigned int volumeIdx=0; volumeIdx < slicerList.size(); volumeIdx++)
    {
        ...
        int iLed = storage_o_layer_size;
        iLed = (int)(config.ledPosition.z/config.layerThickness);
        if ( iLed < storage_b_layer_size)
        {
            // show message "scale up to put bettery and led"
        }
        double intervalX = (config.ledPosition.x-config.objectPosition.X)/(double)(iLed-storage_b_layer_size-1);
                    // (config.ledPosition.x-config.objectPosition.X)/(storage_o_layer_size-storage_l_layer_size-storage_b_layer_size); 
        double intervalY = (config.ledPosition.y-config.objectPosition.Y)/(double)(iLed-storage_b_layer_size-1);
                    // (config.ledPosition.y-config.objectPosition.Y)/(storage_o_layer_size-storage_l_layer_size-storage_b_layer_size);
        printf("iLed: %d, intervalX: %f, intervalY: %f \n", iLed, intervalX, intervalY);
        double intpOffsetX = 0;
        double intpOffsetY = 0;        
        ...
        {   // log("layerNr: %d ", layerNr);    
            ...
            printf("intpOffsetX: %f, intpOffsetY: %f\n", intpOffsetX, intpOffsetY);
            if (layerNr > storage_b_layer_size && layerNr < iLed )
            {
            ...
            {   // log("p1: %d ", p1);
                ...
                LoadFromFile(holeWire1, (char*)"./parts/holewire.txt", 1, holeOffsetX, holeOffsetY);
                LoadFromFile(holeWire2, (char*)"./parts/holewire.txt", 1, holeOffsetX, holeOffsetY);
                // LoadFromFile(holeWire1, (char*)"./parts/holeLed.txt", 0.5, holeOffsetX, holeOffsetY);
                LoadFromFile(holeLed, (char*)"./parts/holeLed.txt", 1, 0, 0);
                // hole set test1 
                
                int objectHoleOffsetX = config.objectPosition.X + (int)intpOffsetX;
                int objectHoleOffsetY = config.objectPosition.Y + (int)intpOffsetY;
...
int main(int argc, char **argv)
{
    ...
    fprintf(stdout,"W_Engine version %s\n", VERSION);
